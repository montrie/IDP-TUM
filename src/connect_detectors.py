...
import pickle
import logging
import fiona  # HINT: this import needs to happen before osmnx, not sure why though
...
from match import match  # HINT: match seems to be a 'soft' keyword since python3.10 -> maybe an issue?
from osmnx.distance import nearest_edges
from taxicab.distance import shortest_path
from config import ROOT_DIR, OSMNX_NETWORK_PLACE, OSMNX_NETWORK_TYPE, OSMNX_SIMPLIFY
...
def delete_network_files(files: [str]):
    """
    Delete files defined in the 'files' list
    :param files: List of file names that will be deleted
    """    
    for file in files:
...
def save_graph_shapefile_directional(graph: MultiDiGraph, filepath=None, encoding="utf-8"):
    """
    Saves the nodes and edges from the graph as shapefiles
    Adapted from https://github.com/cyang-kth/osm_mapmatching/blob/master/README.md#1-download-routable-network-from-osm
    :param graph: A MultiDiGraph network that we want to save
    :param filepath: The path to which we want to write the shapefiles. Defaults to None
    :param encoding: The encoding of the files. Defaults to utf-8
    """
    delete_network_files(networkShapeFiles)
    # default filepath if none was provided
    if filepath is None:
        # redefine the default filepath
        filepath = os.path.join(ox.settings.data_folder, "shapefiles")
...
def get_base_graphml() -> MultiDiGraph:
    """
    Create a base map of munich, add detector static data pulled with pull_static_mobilithek.py
    https://github.com/cyang-kth/osm_mapmatching
    :return: The downloaded MultiDiGraph network of the city
    """
    # Create/load base map
    if not os.path.exists(path_map_munich):
        graph = ox.graph_from_place(OSMNX_NETWORK_PLACE, network_type=OSMNX_NETWORK_TYPE, simplify=OSMNX_SIMPLIFY)
        ox.save_graphml(graph, path_map_munich)
    else:
        graph = ox.load_graphml(path_map_munich)

    # this saves the nodes and edges of 'graph' to separate shapefiles, probably useless since we use gpkg now
...
def connect_detector_nodes(G: MultiDiGraph, detector_nodes: [(float, float)], detector_ids: [int]):
    ...
    len_edges = len(edges)
    # keep track of the number of detectors lying on the same edge
    count = 0
    # keep track of the original edges that need to be deleted at the end
    delete = []
    ...
    for idx, e in enumerate(edges):
        ...
        u, v, key = e
        # get edge attributes of the current node
        edge_attrs = G[u][v][key]

        # add original edge to the list of edges to be deleted
        ...
        if 'geometry' not in edge_attrs:#[0]:
            ...
            edge_attrs['geometry'] = LineString([[start_node['x'], start_node['y']], [end_node['x'], end_node['y']]])
        ...
        for i, p in enumerate([Point(det[0], det[1]) for det in dets]):
            dets[i].append(edge_attrs['geometry'].project(p))
        ...
        line = edge_attrs['geometry']  # retrieve LineString stored in geometry of current edge
        new_geoms = []
        ...
        for i, split_point in enumerate([Point(sd[0], sd[1]) for sd in sorted_dets]):
            ...
            # find point on line that is exactly dist_along_line away from start
            point_on_line = line.interpolate(dist_along_line) 
            # snap the point to the line
            point_on_line = snap(point_on_line, line, 1e-8)
            # From (start, end) we want to get [(start, split_point), (split_point, end)]
# HINT: we might have edge cases in the future where the split functions returns a GeometryCollection object that contains only 1 Linestring
# reference this post: https://stackoverflow.com/questions/50194077/shapely-unable-to-split-line-on-point-due-to-precision-issues
# in that case we could try to create a buffer around point which creates 3 linestrings when executing split
# this is why we extract the coords of geoms[0] and geoms[-1] instead of geoms[1] to make it easier if that edge case comes up
            ...
            split1 = LineString(split1)
            split2 = LineString(split2)
...
# HINT: pull i == 0 case before loop and i == count - 1 case outside of loop, reduce loop to range(1, count-1), makes this more readable
        for i in range(count):
            ...
            if i == 0:
               # shortest_path() uses soon-to-be deprecated function great_circle_vec(), OSMnx 2.0 will rename this to great_circle(); fixed by hand
                ...
                G.add_edge(u, sorted_dets[i][2], **{**edge_attrs, 'geometry': new_geoms[i], 'length': round(dist, 3), 'flow': sorted_dets[i][3]})
            else:
                dist = shortest_path(G, (float(sorted_dets[i-1][1]), float(sorted_dets[i-1][0])), (float(sorted_dets[i][1]), float(sorted_dets[i][0])), e, e)[0]
                G.add_edge(sorted_dets[i-1][2], sorted_dets[i][2], **{**edge_attrs, 'geometry': new_geoms[i], 'length': round(dist, 3), 'flow': sorted_dets[i][3]})
            if i == count - 1:
                dist = shortest_path(G, (float(sorted_dets[i][1]), float(sorted_dets[i][0])), (end_node['y'], end_node['x']), e, e)[0]
                G.add_edge(sorted_dets[i][2], v, **{**edge_attrs, 'geometry': new_geoms[i+1], 'length': round(dist, 3), 'flow': sorted_dets[i][3]})

    # remove the original edges that we split up
    G.remove_edges_from(delete)


def split_and_reconnect():
    """
    Get a base map of Munich using osmnx, match the merged detector locations using a map matching algorithm
    (here: stmatch), add the matched location to the base network
    """
    nodes_list = []
    flow_list = []
    ...
    # call map matching in match.py to align detector nodes with network, the result is written to matched.csv
    match("points.csv")
    ...
    # HINT: matched_detector_locations comes from matched.csv, there might be a mismatch in detector ids between that csv and the xml one
    # clean it somehow
    xml_ids = set(df_xml["detid"].tolist())
    for node_id, node_geom in matched_detector_locations.values:
        if node_id not in xml_ids:
            missing_ids.append(node_id)
            continue
        flow = flows.loc[node_id, 'flow']
        if flow < 0:
            flow = 0

        lon_lat = node_geom.split(',')[0][11:].split(' ')
        flow_list.append(flow)
        lon, lat = lon_lat[0], lon_lat[1]
        # sometimes,  LINESTRING(lon lat) instead of LINESTRING(lon lat,lon lat) -> remove the paranthesis at the end of lat
        if lat[-1] == ')':
            logging.debug(f"node_geom looked like LINESTRING(lon lat) instead of LINESTRING(lon lat,lon lat) for lat: {lat}, node_geom: {node_geom}")
            lat = lat[:-1]
        nodes_list.append((lat, lon))
        detector_ids.append(node_id)
        # add x, y, flow information to new detector nodes in an osmnx network
        nodes_map.add_node(node_id, x=lon, y=lat, flow=flow)

    # create a node dict that sets the size of a node depending on its flow value -> effectively hides non-detector nodes
